...
<div class="container-fluid max-w-4xl">
    <div class="d-flex items-center gap-3 mb-4">
        ...
        <h1 class="h3 font-bold text-slate-900 dark:text-white mb-0">Edit Log #{{ $log->id }}</h1>
    </div>

    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <form action="{{ route('notifications.update', $log->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-warning text-white rounded-pill px-4">
                        <i class="bi bi-pencil me-1"></i> Update Log
                    </button>
                </div>
            </form>
        </div>
