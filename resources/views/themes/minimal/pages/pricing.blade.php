...
@section('title', 'Minimal Pricing')

@section('content')
    <section class="py-5 text-center position-relative">
        <div class="container py-5">
            <h1 class="text-dark fw-bold display-4 mb-3">Subscription Access Plans</h1>
            <p class="text-muted lead mx-auto" style="max-width: 600px;">Unlock resource bounds and scaling modules with no hidden cost matrices.</p>
        </div>
    </section>
                ...
                @foreach($pricing as $plan)
                    <div class="col-lg-4 col-md-6">
                        <div class="mkt-card p-5 h-100 d-flex flex-column position-relative {{ $plan['popular'] ? 'border-primary' : '' }}"
                            ...
                            @if($plan['popular'])
                                <span class="badge bg-primary position-absolute top-0 end-0 translate-middle-y me-5 px-3 py-2 rounded-pill" 
                            ...
                            <div class="mb-4">
                                <h3 class="h3 fw-bold text-dark mb-2">{{ $plan['name'] }}</h3>
                                <p class="small text-muted">{{ $plan['description'] }}</p>
                                <div class="d-flex align-items-baseline text-dark my-4">
    ...
    <section class="py-5 border-top" style="border-color: #e2e8f0 !important;">
        <div class="container" style="max-width: 800px;">
            <h2 class="h3 fw-bold text-dark text-center mb-5">Frequently Asked Questions</h2>
            <div class="d-flex flex-column gap-4">
                <div class="mkt-card p-4">
                    <h4 class="h6 fw-bold text-dark mb-2">Can I upgrade or downgrade my tier?</h4>
                ...
                <div class="mkt-card p-4">
                    <h4 class="h6 fw-bold text-dark mb-2">What is the telemetry limit reset policy?</h4>
